...
```js
desc keyspaces; //listázza az elérhető kulcstereket (adatvbóóbázisokat) --> nekünk csak a movies kell
use movies;
desc tables; //táblák nevének megnévzése
select count(*) from movies_and_tv
desc movies_and_tv //táblák sorainak megnzésée
```
...
```js
insert into szemely (nev, szulev, foglalkozas) values ('Kiss Béla', 2000, 'Lakatos'); //ha lekérem foglalkozás (cluster key) alapján rendezi sorba
```
...
on szemely(foglalkozas); //másodlagos index
...
select nev
from szemely
where foglalkozas IN ['pincer', 'lakatos']; // lehet, hogy sima zárójel kell
```
